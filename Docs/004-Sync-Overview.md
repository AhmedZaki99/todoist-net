...
| full_sync_date_utc | For full syncs, the time when the data was generated. For big accounts, the data may be returned with some delay, requiring an [incremental sync](004-Sync-Overview) to get up-to-date data. |
| user | A user object. |
| projects | An array of [project](012-Sync-Projects) objects. |
| items | An array of [item](#tag/Sync/Items) objects. |
| notes | An array of [task comments](#tag/Sync/Comments/Task-Comments) objects. |
| project_notes | An array of [project comments](013-Sync-Comments) objects. |
| sections | An array of [section](010-Sync-Sections) objects. |
| labels | An array of [personal label](015-Sync-Labels) objects. |
| filters | An array of [filter](017-Sync-Filters) objects. |
| workspace_filters | An array of [workspace filter](018-Sync-Workspace-Filters) objects. |
| day_orders | A JSON object specifying the order of items in daily agenda. |
| reminders | An array of [reminder](011-Sync-Reminders) objects. |
| collaborators | A JSON object containing all [collaborators](009-Sync-Sharing) for all shared projects. The `projects` field contains the list of all shared projects, where the user acts as one of collaborators. |
...
| user_settings | A JSON object containing [user settings](008-Sync-User). |
| user_plan_limits | A JSON object containing [user plan limits](008-Sync-User). |
| stats | A JSON object containing [user productivity stats](008-Sync-User) with completion counts for today and this week. |
| view_options | An array of [view options](#tag/Sync/View-options) objects. |
| project_view_options_defaults | An array of [project view options defaults](007-Sync-Project-View-Options-Defaults) objects. |
| role_actions | The actions each role in the system are allowed to perform on a project |
| workspaces | A JSON object containing [workspace](005-Sync-Workspace) objects. |
