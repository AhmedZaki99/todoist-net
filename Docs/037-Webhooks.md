...
The webhook for a specific user is activated when that user completes the [OAuth flow](002-Authorization) of the app that declares the webhook.
...
1. Performing the [authorization request](002-Authorization) in the browser and capturing the `code` via the browser's developer tools.
2. Performing the [token exchange request](002-Authorization) through a tool like [Postman](https://www.postman.com/) and reading the `access_token` from the response. *Note that you can't make this request via the browser as it needs to be a POST request.*
...
| --- | --- | --- |
| item:added | A task was added | The new [Task](016-Sync-Tasks). |
| item:updated | A task was updated | The updated [Task](016-Sync-Tasks). |
| item:deleted | A task was deleted | The deleted [Task](016-Sync-Tasks). |
| item:completed | A task was completed | The completed [Task](016-Sync-Tasks). |
| item:uncompleted | A task was uncompleted | The uncompleted [Task](016-Sync-Tasks). |
| note:added | A comment was added | The new [Comment](013-Sync-Comments). |
| note:updated | A comment was updated | The updated [Comment](013-Sync-Comments). |
| note:deleted | A comment was deleted | The deleted [Comment](013-Sync-Comments). |
| project:added | A project was added | The new [Project](012-Sync-Projects). |
| project:updated | A project was updated | The updated [Project](012-Sync-Projects). |
| project:deleted | A project was deleted | The deleted [Project](012-Sync-Projects). |
| project:archived | A project was archived | The archived [Project](012-Sync-Projects). |
| project:unarchived | A project was unarchived | The unarchived [Project](012-Sync-Projects). |
| section:added | A section was added | The new [Section](010-Sync-Sections). |
| section:updated | A section was updated | The updated [Section](010-Sync-Sections). |
| section:deleted | A section was deleted | The deleted [Section](010-Sync-Sections). |
| section:archived | A section was archived | The archived [Section](010-Sync-Sections). |
| section:unarchived | A section was unarchived | The unarchived [Section](010-Sync-Sections). |
| label:added | A label was added | The new [Label](015-Sync-Labels). |
| label:deleted | A label was deleted | The deleted [Label](015-Sync-Labels). |
| label:updated | A label was updated | The updated [Label](015-Sync-Labels). |
| filter:added | A filter was added | The new [Filter](017-Sync-Filters). |
| filter:deleted | A filter was deleted | The deleted [Filter](017-Sync-Filters). |
| filter:updated | A filter was updated | The updated [Filter](017-Sync-Filters). |
...
| Event Name | Description | Event Data |
| --- | --- | --- |
| item:updated | For events issued by the user directly these include `old_item` and `update_intent` | `old_item` will be an [Task](016-Sync-Tasks), and `update_intent` can be `item_updated`, `item_completed`, `item_uncompleted`. |
...
| --- | --- |
| event_name *String* | The event name for the webhook, see the table in the [Configuration](037-Webhooks) section for the list of supported events. |
| user_id *String* | The ID of the user that is the destination for the event. |
| event_data *Object* | An object representing the modified entity that triggered the event, see the table in the [Configuration](037-Webhooks) section for details of the `event_data` for each event. |
...
| event_data_extra *Object* | Optional object that can include meta information, see the table in the [Configuration](037-Webhooks) section for details of the `event_data_extra` for each event. |
